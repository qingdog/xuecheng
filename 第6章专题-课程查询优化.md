...
public CoursePublish getCoursePublishCache(Long courseId){
    //查询缓存
    String jsonString = (String) redisTemplate.opsForValue().get("course\_" + cou
    if (StringUtils.isNotEmpty(jsonString)) {
       ...
       if(coursePublish!=null){
           redisTemplate.opsForValue().set("course\_" + courseId, JSON.toJSONStr
        }
        return coursePublish;
    }
}
...
public CoursePublish getCoursePublishCache(Long courseId) {
	//查询缓存
	String jsonString = (String) redisTemplate.opsForValue().get("course\_" + courseId);
    ...
    if (StringUtils.isNotEmpty(jsonString)) {
	   ...
	   //设置过期时间300秒
        ...
        redisTemplate.opsForValue().set("course\_" + courseId, JSON.toJSONString(coursePublish),300, TimeUnit.SECONDS);
        return coursePublish;
		}
}
```
...
```java
   //设置过期时间300秒
  redisTemplate.opsForValue().set("course\_" + courseId, JSON.toJSONString(cours
...
public CoursePublish getCoursePublishCache(Long courseId){
       ...
       if(StringUtils.isNotEmpty(jsonString)){
           ...
           synchronized(this){
                jsonString = (String) redisTemplate.opsForValue().get("course\_" + courseId);
               if(StringUtils.isNotEmpty(jsonString)){
                   CoursePublish coursePublish = JSON.parseObject(jsonString,CoursePublish.class);
                   return coursePublish;                }
                System.out.println("=========从数据库查询==========");
...
public CoursePublish getCoursePublishCache(Long courseId){
       ...
       if(StringUtils.isNotEmpty(jsonString)){
           ...
           try {
               ...
               //从数据库查询
               CoursePublish coursePublish = getCoursePublish(courseId);
               redisTemplate.opsForValue().set("course\_" + courseId, JSON.toJSONString(coursePublish),1,TimeUnit.DAYS);
               return coursePublish; 
           }finally {
               //释放锁
               lock.unlock();
